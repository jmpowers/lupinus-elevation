...
popnames = c(`2017`="Lupine 2017",brc="Brush Creek Trailhead",
             lerb="Lower East River Bridge", erb="East River Bridge", bur="Burro Park",
             eml="Emerald Lake", mar="Marmot Mesa",
             bvb="Bellview Bench", mex="Mexican Cut Turnoff",
             elk="Elko Park", `401`="401", 
             npb="North Pole Basin", chs="Currant Hill", BellN="BellN 2018",
             bcn="Bellview Corner", bwb="Bellview Wilderness Boundary",
             ypo="Yule Pass Trailhead", cmn="Cinnamon")
#can't find Gothic Parking Lot abbreviation, perhaps that is LERB (lower east river bridge?). 2018 samples are all labeled BellN. 
...
lupmeta <- read_tsv("data/RMBL GC-MS Data Inventory - maxfield_all210827annot.tsv", na="NA") %>% 
  ...
  mutate(type = ifelse(str_detect(sample,"air"), "ambient", "floral"),
         sample_grid = str_remove(sample, "air_")) %>% 
  ...
  rows_update(read_tsv("data/lupmeta2017thru2021 - lupmeta.tsv", col_types="ccccccccccccccccccc") %>% 
                filter(is.na(index)) %>% 
                select(FileName, type, pop, sample_year, plant, rundate), by="FileName") %>% 
  ...
  mutate(population = factor(recode(pop, !!!popnames), levels=popnames))
